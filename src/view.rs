...
pub trait View: Sync + Send + 'static {
    ...
    {
        ViewRoot(Arc::new(Mutex::new(ViewState {
            view: self,
            state: None,
        })))
    }
...
pub trait AnyViewState: Sync + Send + 'static {
    ...
    /// If this ViewState has an owner, return it. The owner is the entity that owns the tracking
    /// scope for the view.
...
// pub struct ViewTemplateView<V: ViewTemplate> {
//     view: Arc<V>,
// }
...
pub(crate) fn rebuild_views(world: &mut World) {
    ...
    let mut scopes = world.query::<(Entity, &mut TrackingScope, &ViewCell)>();
    let changed = scopes
        .iter(world)
    ...
    for scope_entity in changed.iter() {
        ...
        let mut cx = Cx::new(world, *scope_entity, &mut next_scope);
        inner.lock().unwrap().rebuild(&mut cx);
